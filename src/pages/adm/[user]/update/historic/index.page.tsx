...
export default function Historic({ data, username }: HistoricProps) {
  const weekDays = [
    'Domingo',
    'Segunda-feira',
    'Terça-feira',
  ...
  return (
    <HistoryContainer>
      <Box>
        <Link href={`/adm/${username}/update`}>
          <CaretLeft size={24} weight="bold" />
        </Link>
        <h1>Histórico de refeições do usuário - {username}</h1>
      </Box>
      <HistoryList>
        <table>
          <thead>
...
`
  ...
  const api = google.tasks({
    version: 'v1',
    auth: await getGoogleOAuthToken(session.id),
  })
